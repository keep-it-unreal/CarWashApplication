...
@Slf4j
public final class TimeAddCurCommand extends SimpleCommand {

    // обязательно нужно вызвать конструктор суперкласса,
    // передав в него имя и описание команды
    public TimeAddCurCommand(BotUserService botUserService,
                             BotController botController) {
        super("time-add-cur", "Добавить график работы на текущий месяц (с дня, следующего за текущим) для всех моек владельца\n", botUserService, botController);
    }
    ...
    private void whatDo(AbsSender absSender, SendMessage answer, BotUser botUser, User user) {
        ...
        HttpAnswer httpAnswer = botController.addTimeTableForCurrentMonth(botUser.getIdUser());
