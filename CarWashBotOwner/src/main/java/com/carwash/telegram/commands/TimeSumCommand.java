...
@Slf4j
public final class TimeSumCommand extends SimpleCommand {

    // обязательно нужно вызвать конструктор суперкласса,
    // передав в него имя и описание команды
    public TimeSumCommand(BotUserService botUserService,
                          BotController botController) {
        super("time-sum", "Добавить график работы на текущий месяц (с дня, следующего за текущим) для всех моек владельца\n", botUserService, botController);
    }
    ...
    private void whatDo(AbsSender absSender, SendMessage answer, BotUser botUser, User user) {
        ...
        botUserService.setServiceStep( botUser.getId(), BotUserStepService.TIME_SUM);
